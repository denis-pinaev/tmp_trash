...
class Balancer():
    ...
    def faceDetect(self, data_image, id_command = 'SDKBalancer'):
        ...
        if not res: return ("error", images)
    ...
    def learning(self, data_image, id, params={}, id_command = 'SDKBalancer'):
        ...
        if "face" in params:
            xml_file = '<methodCall><methodName>learning</methodName><params><login>%s</login><password>%s</password><user>%s</user><person_name>%s</person_name><face>%s</face></params><id_command>learning%s</id_command></methodCall>'  % (self.login, self.passwd, self.user, id, self.__joinRes(params), id_command)
        else:
    ...
    def setParameter(self, value, param = 'eyes_distance', toBase="", id_command = 'SDKBalancer'):
        
        self.__connect()
        
        if toBase:
            xml_file = '<methodCall><methodName>setparameter</methodName><params><login>%s</login><password>%s</password><user>%s</user><toBase>%s</toBase><parameter>%s</parameter><value>%s</value></params><id_command>setParameter%s</id_command></methodCall>'  % (self.login, self.passwd, self.user, toBase, param, value, id_command)
        else:        
