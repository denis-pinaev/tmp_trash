...
def paginate(context, window=DEFAULT_WINDOW):
    ...
    try:
        ...
        first = set(page_range[:1]) #first = set(page_range[:window])
        last = set(page_range[-1:]) #last = set(page_range[-window:])
        # Now we look around our current page, making sure that we don't wrap
        # around.
        current_start = page_obj.number-1-window
        if current_start < 0:
            current_start = 0
        current_end = page_obj.number-1+window+1 #current_end = page_obj.number-1+window
        if current_end < 0:
            current_end = 0
