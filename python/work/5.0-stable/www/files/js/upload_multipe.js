...
function selectFace(inp) {
    ...
    if (inp.attr("checked")) {
		...
		inp.parent().addClass("lp_act");
    } else {
        count_choose--
		inp.parent().removeClass("lp_act");
        if (count_choose == 0) {
            link_choose.parent().addClass("mcf_dis");
        }
    }
    if (count_choose == 0) count_choose = "0";
	if (count_all == count_choose) $("div.bl_btn_check input[type=checkbox]").attr("checked", "checked");
	else $("div.bl_btn_check input[type=checkbox]").attr("checked", false);
...
function selectAllFaces(inp) {
    ...
    var link_choose = $("#menu_cutfaces li a.link_choose");
    if (inp.attr("checked")) {
		$("div.bl_btn_check input[type=checkbox]").attr("checked", "checked");
		$("#list_photos li").addClass("lp_act");
		$("#list_photos li input[type=checkbox]").attr("checked", "checked");
        count_choose = $("#list_photos li input[type=checkbox]:checked").size();
        link_choose.parent().removeClass("mcf_dis");
    } else {
		$("div.bl_btn_check input[type=checkbox]").attr("checked", false);
...
function showOnlySelectFace() { 
    ...
    $.each(checks, function() {
        content = content + "<li>" + $(this).parent().html() + "</li>";
    });
    list_choose.html(content);
    $("#list_photos_choose li input[type=checkbox]").css("display", "none");
    list_choose.show();
    setupZoom();
    initTooltip();
}
